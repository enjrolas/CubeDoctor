...
#ifndef MAIN_H
  #define EXITM(x) exit(x)
#else
  #define EXITM(x) return (x)
#endif
...
{	return;
	PRINTF_STDERR_MESSAGE("Usage: dfu-util [options] ...\n"
		"  -h --help\t\t\tPrint this help message\n"
		"  -V --version\t\t\tPrint the version number\n"
		"  -v --verbose\t\t\tPrint verbose debug statements\n"
		"  -l --list\t\t\tList currently attached DFU capable devices\n");
	PRINTF_STDERR_MESSAGE("  -e --detach\t\t\tDetach currently attached DFU capable devices\n"
	...
	PRINTF_STDERR_MESSAGE("  -t --transfer-size <size>\tSpecify the number of bytes per USB Transfer\n"
...
{
	PRINTF_MESSAGE(PACKAGE_STRING "\n\n");
	PRINTF_MESSAGE("Copyright 2005-2009 Weston Schmidt, Harald Welte and OpenMoko Inc.\n"
...
//类似于命令行参数，将szStr分隔的指针存于argv[]，nMaxN:最多取的个数,cSeparatorChr:分隔符号(:,.等),返回实际个数
//szStr有分隔符的地方将置为0,以便分隔
//如 "exam.exe -D 1 45 12"  分隔符为' ',则返回5,argv[0]="exam.exe",argv[1]="-D",...
int GetArgvFromStr(IN TCHAR szStr[],OUT TCHAR * argv[],IN int nMaxN,TCHAR cSeparatorChr)
{
	TCHAR *pp;
	TCHAR *p=szStr;
	int m=0;
...
//		errx(EX_IOERR, "unable to initialize libusb: %i", ret);
		sprintf(ss,"unable to initialize libusb: %i", ret);
		if(pszInfo && (nMaxLen==-1||nMaxLen>strlen(ss)))
			strcpy(pszInfo,ss);
		return -1;
	}
// 	if (verbose > 2) 
// 		libusb_set_debug(ctx, 255);
	...
	ret=probe_devices(ctx);	//ret<0:获取设备失败

	fd=MDMSG_NO;
	if(ret>=0)
	{
		fd=list_dfu_interfaces(pszInfo,nMaxLen);
		if(dfu_root)
		{
...
// 	char s1[10],s2[20];
	...
	InitDfuseParam();
	//
	argc=GetArgvFromStr(szCmdLine,argv,20,' ');
//	MessageBox(hWnd,argv[1],argv[0],MB_OK);
// 	s2[0]=argc/10+0x30; s2[1]=argc%10+0x30; s2[2]=0;
// 	MessageBox(hWnd,s2,NULL,MB_OK);
	InitGetopt();
#endif
...
// 		MessageBox(hWnd,s1,s2,MB_OK);
	...
	if (mode == MODE_VERSION) {
		print_version(); //bywbj20151024
		EXITM(0);
	}
	//	print_version(); //bywbj20151024

	if (mode == MODE_NONE) {
		PRINTF_STDERR_MESSAGE("You need to specify one of -D or -U\n");
		help();	EXITM(EX_USAGE);
	}
	...
	if (mode == MODE_DOWNLOAD) {
		...
		if (match_vendor < 0 && file.idVendor != 0xffff) {
			...
			MYPRINTF("Match vendor ID from file: %04x\n", match_vendor);
		}
		if (match_product < 0 && file.idProduct != 0xffff) {
			match_product = file.idProduct;
			MYPRINTF("Match product ID from file: %04x\n", match_product);
		}
	}
...
//		goto exit_dfu;
	...
	MYPRINTF("Opening DFU capable USB device...\n");
	...
	MYPRINTF("Run-time device DFU version %04x\n",
	...
	if (!(dfu_root->flags & DFU_IFF_DFU)) {
		...
		* DFU Interface descriptor according to the DFU Spec. */
		...
		MYPRINTF("Claiming USB DFU Runtime Interface...\n");
		...
		MYPRINTF("Determining device status: ");

		err = dfu_get_status(dfu_root, &status);
		if (err == LIBUSB_ERROR_PIPE) {
			MYPRINTF("Device does not implement get_status, assuming appIDLE\n");
		...
		case DFU_STATE_appDETACH:
			MYPRINTF("Device really in Runtime Mode, send DFU "
			       "detach request...\n");
			...
			if (dfu_root->func_dfu.bmAttributes & USB_DFU_WILL_DETACH) {
				MYPRINTF("Device will detach and reattach...\n");
			} else {
				MYPRINTF("Resetting USB...\n");
		...
		case DFU_STATE_dfuERROR:
			MYPRINTF("dfuERROR, clearing status\n");
		...
		MYPRINTF("Opening DFU USB Device...\n");
		ret = libusb_open(dfu_root->dev, &dfu_root->dev_handle);
		if (ret || !dfu_root->dev_handle) {
			errx(EX_IOERR, "Cannot open device");
		}
...
#endif
	MYPRINTF("Claiming USB DFU Interface...\n");
	if (libusb_claim_interface(dfu_root->dev_handle, dfu_root->interface) < 0) {
		errx(EX_IOERR, "Cannot claim interface");
	}

	MYPRINTF("Setting Alternate Setting #%d ...\n", dfu_root->altsetting);
...
status_again:
	...
	case DFU_STATE_dfuUPLOAD_IDLE:
		MYPRINTF("aborting previous incomplete transfer\n");
	...
	MYPRINTF("DFU mode device DFU version %04x\n",
	       libusb_le16_to_cpu(dfu_root->func_dfu.bcdDFUVersion));
	...
	if (!transfer_size) {
		...
		if (transfer_size) {
			MYPRINTF("Device returned transfer size %i\n",
			       transfer_size);
		} else {
...
/* autotools lie when cross-compiling for Windows using mingw32/64 */
	...
	if ((int)transfer_size > getpagesize()) {
		...
		MYPRINTF("Limited transfer size to %i\n", transfer_size);
	}
#endif /* __MINGW32__ */
	...
	if (transfer_size < dfu_root->bMaxPacketSize0) {
		...
		MYPRINTF("Adjusted transfer size to %i\n", transfer_size);
	}
	...
	if (final_reset) {
		...
		MYPRINTF("Resetting USB to switch back to runtime mode\n");
		ret = libusb_reset_device(dfu_root->dev_handle);
		if (ret < 0 && ret != LIBUSB_ERROR_NOT_FOUND) {
