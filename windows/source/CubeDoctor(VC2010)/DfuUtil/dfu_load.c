    ...
    int expected_size, int fd)
{
	...
	MYPRINTF("Copying data from DFU device to PC\n");
...
out_free:
	...
	free(buf);
	if (verbose)
		MYPRINTF("Received a total of %i bytes\n", total_bytes);
...
int dfuload_do_dnload(struct dfu_if *dif, int xfer_size, struct dfu_file *file)
{
	...
	MYPRINTF("Copying data from PC to DFU device\n");
	...
	while (bytes_sent < expected_size) {
		...
		if (ret < 0) {
			warnx("Error during update");	//download
			goto out;
		}
	...
	if (verbose)
		MYPRINTF("Sent a total of %i bytes\n", bytes_sent);
