...
found_dfu:
		if (func_dfu.bLength == 7) {
			MYPRINTF("Deducing device DFU version from functional descriptor "
			    "length\n");
			func_dfu.bcdDFUVersion = libusb_cpu_to_le16(0x0100);
		} else if (func_dfu.bLength < 9) {
			MYPRINTF("Error obtaining DFU functional descriptor\n");
			MYPRINTF("Please report this as a bug!\n");
			MYPRINTF("Warning: Assuming DFU version 1.0\n");
			func_dfu.bcdDFUVersion = libusb_cpu_to_le16(0x0100);
			MYPRINTF("Warning: Transfer size can not be detected\n");
...
int probe_devices(libusb_context *ctx)
{
	libusb_device **list;
	ssize_t num_devs;
	ssize_t i;
	...
	if(num_devs>=0)
		m=1;
	libusb_free_device_list(list, m);//0);
	return num_devs;
}
...
void print_dfu_if(struct dfu_if *dfu_if,__out char *pszInfo)
{
	if(pszInfo==NULL)
		return;
	sprintf(pszInfo,"Found %s: [%04x:%04x] ver=%04x, devnum=%u, cfg=%u, intf=%u, "
...
int list_dfu_interfaces(__out char *pszInfo,int nMaxLen)
{
	...
	{
		szCur[0]=0;
		print_dfu_if(pdfu,szCur);
		if(pszInfo)
		{
			l0=strlen(szCur);
			if(ln+l0<nMaxLen||nMaxLen==-1)
			{
				strcat(pszInfo,szCur);	ln+=l0;
			}
		}
		else if(szCur[0]!=0)
		{
			PRINTF_MESSAGE(szCur);
		}
		if(pdfu->flags & DFU_IFF_DFU) //? "DFU" : "Runtime"
			n++;
		m++;
	}
	if(n>0)
		return MDMSG_DFU; //SEND_DFU_MESSAGE(MDMSG_DFU);
	else if(m!=0&&n==0)
		return MDMSG_RUNTIME; //SEND_DFU_MESSAGE(MDMSG_RUNTIME);
//	else if(m==0)
	return MDMSG_NO; //SEND_DFU_MESSAGE(MDMSG_NO);
}
