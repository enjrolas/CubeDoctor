			  ...
			  enum dfuse_command command)
{
	...
	if (command == ERASE_PAGE) {
		...
		if (verbose > 1)
			MYPRINTF("Erasing page size %i at address 0x%08x, page "
			       "starting at 0x%08x\n", page_size, address,
			       address & ~(page_size - 1));
		...
		if (verbose > 2)
			MYPRINTF("  Setting address pointer to 0x%08x\n",
			       address);
	...
	if (ret < 0) {
		errx(EX_IOERR, "Error during special command \"%s\" update",
	...
	do {
		...
		if (firstpoll) {
			...
			if (dst.bState != DFU_STATE_dfuDNBUSY) {
				MYPRINTF("state(%u) = %s, status(%u) = %s\n", dst.bState,
				       dfu_state_to_string(dst.bState), dst.bStatus,
				       dfu_status_to_string(dst.bStatus));
				errx(EX_IOERR, "Wrong state after command \"%s\" update",
				     dfuse_command_name[command]);
			}
		}
		/* wait while command is executed */
		if (verbose)
			MYPRINTF("   Poll timeout %i ms\n", dst.bwPollTimeout);
		       ...
		       int transaction)
{
	...
	if (ret < 0) {
		errx(EX_IOERR, "Error during update");
		return ret;
	}
	bytes_sent = ret;
	...
	do {
		ret = dfu_get_status(dif, &dst);
		if (ret < 0) {
			errx(EX_IOERR, "Error during update get_status");
			return ret;
		}
		milli_sleep(dst.bwPollTimeout);
	...
	if (dst.bState == DFU_STATE_dfuMANIFEST)
			MYPRINTF("Transitioning to dfuMANIFEST state\n");
		    ...
		    const char *dfuse_options)
{
	...
	if (dfuse_address) {
		...
		if (!upload_limit) {
			...
			MYPRINTF("Limiting upload to end of memory segment, "
			       "%i bytes\n", upload_limit);
		}
		...
		if (!upload_limit)
			upload_limit = 0x4000;
		MYPRINTF("Limiting default upload to %i bytes\n", upload_limit);
	}
...
//	dfu_progress_bar("Download", 0, 1);
	PRINTF_MESSAGE("Updating...");
	SET_PROGRESS_RANGE_MESSAGE(dwElementSize);
	...
	for (p = 0; p < (int)dwElementSize; p += xfer_size) {
		...
		if ((segment->memtype & DFUSE_ERASABLE) && !dfuse_mass_erase) {
			...
			if (((address + chunk_size - 1) & ~(page_size - 1)) !=
			    last_erased_page) {
				if (verbose > 2)
					MYPRINTF(" Chunk extends into next page,"
					       " erase it as well\n");
				dfuse_special_command(dif,
...
//		dfu_progress_bar("Download", dwElementSize, dwElementSize);
		SET_PROGRESS_CURPOS_MESSAGE(dwElementSize);
		PRINTF_MESSAGE("\nUpdate done.\n");
	}
	return 0;
}
			...
			struct dfu_file *file, unsigned int start_address)
{
	...
	MYPRINTF("Updating to address = 0x%08x, size = %i\n",
	...
	MYPRINTF("File updated successfully\n");
	ret = dwElementSize;
			  ...
			  struct dfu_file *file)
{
	...
	MYPRINTF("file contains %i DFU images\n", bTargets);

	for (image = 1; image <= bTargets; image++) {
		MYPRINTF("parsing DFU image %i\n", image);
		...
		MYPRINTF("image for alternate setting %i, ", bAlternateSetting);
		MYPRINTF("(%i elements, ", dwNbElements);
		MYPRINTF("total size = %i)\n",
		       quad2uint((unsigned char *)targetprefix + 266));
		if (bAlternateSetting != dif->altsetting)
			MYPRINTF("Warning: Image does not match current alternate"
			       " setting.\n"
			       "Please rerun with the correct -a option setting"
			       " to update this image!\n");
		for (element = 1; element <= dwNbElements; element++) {
			MYPRINTF("parsing element %i, ", element);
			...
			MYPRINTF("address = 0x%08x, ", dwElementAddress);
			MYPRINTF("size = %i\n", dwElementSize);
	...
	MYPRINTF("done parsing DfuSe file\n");
		    ...
		    const char *dfuse_options)
{
	...
	if (dfuse_unprotect) {
		...
		MYPRINTF("Device disconnects, erases flash and resets now\n");
		exit(0);
	}
	if (dfuse_mass_erase) {
		if (!dfuse_force) {
		...
		MYPRINTF("Performing mass erase, this can take a moment\n");
