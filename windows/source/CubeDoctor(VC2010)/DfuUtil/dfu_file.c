		...
		unsigned long long max)
{
	...
	MYPRINTF("\r%s\t[%s] %3lld%% %12lld bytes", desc, buf,
...
void dfu_load_file(struct dfu_file *file, enum suffix_req check_suffix, enum prefix_req check_prefix)
{
	...
	if (!strcmp(file->name, "-")) {
		...
		if (verbose)
			MYPRINTF("Read %i bytes from stdin\n", file->size.total);
...
checked:
	...
	if (file->size.prefix && verbose) {
		...
		if (file->prefix_type == LMDFU_PREFIX)
			MYPRINTF("Possible TI Stellaris DFU prefix with "
			...
			MYPRINTF("Possible unencrypted NXP LPC DFU prefix with "
				   "the following properties\n"
...
void show_suffix_and_prefix(struct dfu_file *file)
{
	if (file->size.prefix == LMDFU_PREFIX_LENGTH) {
		MYPRINTF("The file %s contains a TI Stellaris DFU prefix with the following properties:\n", file->name);
		MYPRINTF("Address:\t0x%08x\n", file->lmdfu_address);
	} else if (file->size.prefix == LPCDFU_PREFIX_LENGTH) {
		uint8_t * prefix = file->firmware;
		MYPRINTF("The file %s contains a NXP unencrypted LPC DFU prefix with the following properties:\n", file->name);
		MYPRINTF("Size:\t%5d kiB\n", prefix[2]>>1|prefix[3]<<7);
	} else if (file->size.prefix != 0) {
		MYPRINTF("The file %s contains an unknown prefix\n", file->name);
	}
	if (file->size.suffix > 0) {
		MYPRINTF("The file %s contains a DFU suffix with the following properties:\n", file->name);
		MYPRINTF("BCD device:\t0x%04X\n", file->bcdDevice);
		MYPRINTF("Product ID:\t0x%04X\n",file->idProduct);
		MYPRINTF("Vendor ID:\t0x%04X\n", file->idVendor);
		MYPRINTF("BCD DFU:\t0x%04X\n", file->bcdDFU);
		MYPRINTF("Length:\t\t%i\n", file->size.suffix);
		MYPRINTF("CRC:\t\t0x%08X\n", file->dwCRC);
	}
}
